...
class AveragePriceMonthlyCatalog < ApplicationService
  ...
  def call
    match = self.class.find_price(slug: @slug, year: @year, month: @month)
    match ? success(match) : failure(code: :price_not_found, message: "No monthly price found")
  end
end
